...
<div class="wrap">
  ...
  <section class="metric">
    ...
    <p class="metric-def">Среднесуточное число завершённых поездок (все 4 типа). Единственная метрика, напрямую отражающая объём доставленной ценности — совпадения спроса и предложения на рынке.</p>
    ...
    это всё ещё <b>&minus;20% от пикового 2019</b>. Это должна быть метрика №1 в еженедельном обзоре: любая инициатива роста должна двигать именно её,
    а не суррогаты вроде GMV, которые растут за счёт цены (см. Fare per Mile ниже), а не реального объёма спроса.</p>
  </section>
  ...
  <section class="metric">
    ...
    восстановление роста шло за счёт возврата водителей на платформу, а не за счёт перегрузки существующего супплая. Если этот guardrail
    начнёт расти вместе с NSM &mdash; это будет ранний сигнал, что рост уже упирается в потолок предложения.</p>
  </section>
  ...
  <section class="metric">
    ...
    начиная с 2023 года &mdash; это прямое отражение введённого TLC минимального стандарта оплаты водителей (driver pay floor). Продуктовый вывод:
    для этого рынка guardrail по оплате водителя <b>защищён регуляторно</b>, а не только решением платформы &mdash; значит unit-экономика роста NSM
    должна закладывать этот пол как жёсткое ограничение, а не как переменную для оптимизации маржи.</p>
  </section>
  ...
  <section class="metric">
    ...
    <p class="insight">Растёт почти монотонно, <b>+63.5%</b> за 7 лет &mdash; быстрее, чем оплата водителя (+29%), значит маржа платформы на милю,
    ...
    или на wait time &mdash; ранний тревожный сигнал перед тем, как рост начнёт вымывать чувствительных к цене райдеров.</p>
  </section>
  ...
  <section class="metric">
    ...
    <p class="insight">Самое интересное расхождение во всём наборе: ликвидность рынка <b>уже полностью восстановилась</b> до уровня 2019 (даже
    чуть выше), при этом число активных зон не изменилось (~252 стабильно) &mdash; но NSM всё ещё на 20% ниже пика 2019. Значит проблема роста
    ...
    меньше поездок, чем до пандемии (структурный эффект удалённой работы). Не путать эту метрику с NSM: здоровый proxy при недовосстановленном
  ...
  <div class="synth">
    ...
    Guardrail-ы показывают, что восстановление роста было <b>здоровым</b> &mdash; не за счёт водителей (оплата растёт) и не за счёт качества сервиса
