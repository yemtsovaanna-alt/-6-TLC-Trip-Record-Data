...
RULE_NOTES = {
    "yellow": [
        ...
        "а не брак данных",
        "passenger_count = 0 / NULL НЕ фильтруется: как и раньше, это распространённый "
        "(до ~8%) пробел в отчётности части вендоров, а не признак повреждённой записи",
    ],
    "green": [
...
def clean_one_file(taxi_type: str, raw_path_str: str) -> dict:
    ...
    # ".tmp" на процесс приводил к коллизиям имён спилл-файлов между параллельными
...
def write_log(discovery: dict, apply_results: list[dict], validate_results: list[dict],
    ...
    if apply_results:
        lines.append("## Применённая очистка (файлы, обработанные в этом запуске)\n")
...
def main():
    ...
    # всё равно проверит вообще все 356 файлов на соответствие правилам.
    def is_valid_parquet(p: Path) -> bool:
        # Дешёвая проверка читаемости footer'а — ловит файлы, побитые предыдущими
        # крашами системы (напр. rename прошёл, но данные не успели сброситься на диск).
        try:
            con = duckdb.connect()
    ...
    for t in ["yellow", "green", "fhv", "fhvhv"]:
        ...
        for p in existing:
            if is_valid_parquet(p):
                ...
                print(f"  Обнаружен повреждённый файл, будет пересоздан: {p}")
